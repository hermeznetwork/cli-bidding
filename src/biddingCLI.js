...
async function main() {
    ...
    const HermezAuctionContract = new ethers.Contract(addressSC[network.chainId].hermezAuctionAddress, artifactAuction.abi, provider);

    const artifactHEZ= require(path.join(__dirname, "../config/artifacts/ERC20Permit.json"));
    const HezContract = new ethers.Contract(addressSC[network.chainId].hezTokenAddress, artifactHEZ.abi, provider);
    ...
    if (command === "CURRENTWINNINGBIDS") {
        ...
        if (Object.keys(slots).length > 0 ) {
            console.log("Current winning bids:");
            Object.keys(slots).forEach( slot => {
                console.log(`Slot : ${slot} bidAmount: ${ethers.utils.formatEther(slots[slot])} HEZ`);
            });
        } else {
            console.log("Currently there's no winning bids");
        }
